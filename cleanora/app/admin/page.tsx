...
const QUOTE_STATUS_OPTIONS: readonly ("pending" | "accepted" | "rejected")[] = ["pending", "accepted", "rejected"] as const
...
export default function AdminDashboard(): JSX.Element {
  ...
  const fetchCompanies = useCallback(async (): Promise<void> => {
    ...
    try {
      const { data, error } = await supabase.from("cleaning_companies").select("*").order("company_name", { ascending: true })
      if (error) throw error
      // Ensure company IDs are properly converted to numbers for bigint columns
  ...
  const fetchQuotes = useCallback(async (leadId: string): Promise<void> => {
    ...
    try {
      ...
      setQuotes(
        ((data as any[]) || []).map((q) => ({
          ...q,
          company_name: q.company_id?.company_name || "Unbekannt",
        }))
      )
    } catch (err: unknown) {
      console.error(err)
  ...
  useEffect(() => {
    if (selectedLead) {
      ...
      // Convert company_id to string for select, handle bigint properly
  ...
  // FIXED: Assign company to lead - ensures correct company_id is saved
  const handleAssignCompany = async (): Promise<void> => {
    if (!selectedLead || assigningCompany) return
    setAssigningCompany(true)
    try {
      // Get the actual selected company ID from the companies array
      let companyId: number | null = null
      if (modalCompanyId) {
        // Find the company by matching the string value to ensure we have the correct ID
        const selectedCompany = companies.find(c => String(c.id) === modalCompanyId)
        if (selectedCompany) {
          companyId = selectedCompany.id
        } else {
          // Fallback: try direct number conversion
      ...
      // Use correct column name 'company_id' and ensure it's a proper number
      const { error } = await supabase
        .from("leads")
        .update({
      ...
      // Update local state with correct column name
      setLeads((prev) =>
        prev.map((l) =>
          l.id === selectedLead.id
      ...
      // Refresh leads to ensure data consistency with database
      await fetchLeads()
  ...
  const handleArchiveLead = async (): Promise<void> => {
    ...
    if (!window.confirm("Möchten Sie diesen Lead wirklich archivieren?")) {
      return
    }
  ...
  // Close lead modal and reset state
  ...
  // Close company modal and reset form
  ...
  // Get company name by ID - Handle both string and number comparison for bigint IDs
  const getCompanyNameById = (companyId: number | null): string => {
    if (!companyId) return "-"
    // Try exact match first, then string comparison for bigint handling
                  ...
                  <div className="p-6">
                    {leadModalTab === "details" ? (
                      <>
                        ...
                        {/* Company Assignment Section - FIXED */}
                          ...
                          <div className="flex gap-2">
                            ...
                            >
                              ...
                              {companies.filter(c => c.active).map((c) => (
                                // Use actual company.id as option value, converted to string for HTML select
                      ...
                      <>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          <div>
                            <h3 className="text-lg font-semibold text-emerald-400 mb-4">Passende Firmen ({getMatchingCompanies(selectedLead).length})</h3>
                            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                              {getMatchingCompanies(selectedLead).length === 0 && (
                                <p className="text-slate-500 text-sm italic">Keine passenden Firmen gefunden (gleiche Stadt & Service).</p>
                              )}
                              {getMatchingCompanies(selectedLead).map((c) => (
                                <div key={c.id} className="p-4 bg-slate-800/40 rounded-xl border border-slate-700 flex justify-between items-start gap-3">
                                  <div>
                                    <p className="font-semibold text-slate-200">{c.company_name}</p>
                                    <p className="text-xs text-slate-400 mt-1">{c.city} • {parseArray(c.services).slice(0,3).join(", ")}{parseArray(c.services).length > 3 ? "..." : ""}</p>
                                  </div>
                                  <button 
                                    onClick={() => handleCreateQuote(c.id)} 
                                    disabled={sendingQuote === String(c.id) || quotes.some(q => q.company_id === c.id)}
                                    className="px-3 py-1.5 rounded bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 disabled:text-slate-500 text-xs font-medium transition whitespace-nowrap"
                                  >
                                    {sendingQuote === String(c.id) ? "Sendet..." : quotes.some(q => q.company_id === c.id) ? "Gesendet" : "Anfrage senden"}
                                  </button>
                                </div>
                              ))}
                            </div>
                          ...
                          <div>
                            <h3 className="text-lg font-semibold text-blue-400 mb-4">Anfragen & Angebote</h3>
                            ...
                            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                              {quotes.map((q) => (
                                <div key={q.id} className="p-4 bg-slate-800/40 rounded-xl border border-slate-700">
                                  {editingQuoteId === q.id ? (
                                    <div className="space-y-2">
                                    ...
                                    <div className="flex justify-between items-start gap-3">
                                      <div>
                                        ...
                                        <p className="text-xs text-slate-400 mt-1">Status: <span className="capitalize font-medium text-blue-300">{q.status}</span> • {q.price ? `${q.price.toFixed(2)} €` : "Kein Preis"}</p>
                                        {q.message && <p className="text-xs text-slate-500 mt-1 italic bg-slate-900/50 p-2 rounded mt-2">"{q.message}"</p>}
                                      </div>
